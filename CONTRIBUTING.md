...
本リポジトリでは、人間とAIエージェントを同じ開発規範の対象として扱います。ただし、役割と意思決定権は区別します。

- **人間のAdjudicator（審判者）**: フェーズ移行、Phase 1テスト、ADR、曖昧な仕様、外部サービスやライセンスの採否を判断する。
- **AIエージェント**: 設計 intake、最小コンテキスト選択、仕様に沿った成果物作成、検証、前提と未決事項の記録を行う。自分の成果物を自分の判断だけで承認しない。
- **決定論的ツール**: テスト、静的解析、CodeQL、フォーマッターなど、モデル判断に依存しない検証信号を提供する。

すべての役割は同じ運用規範、安全性基準、受入条件に従いますが、Adjudicatorの承認権限をAIエージェントへ移譲しません。
