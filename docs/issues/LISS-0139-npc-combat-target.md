...
- `NpcAiPlanner`をField非依存の決定論的Plannerとして追加し、観測値から固定ステップの
  移動Decisionだけを生成するようにした。Decisionは`NpcAiInputQueue`へ入り、
  `WorldFrameApplier`がTick内で検証・位置キューへ変換する。PlannerはCombat、乱数、
  時刻、外部I/O、Fieldの直接変更を行わない。
...
- Open decisions: NPC発の攻撃・魔法、戦闘AI、乱数provider、時刻provider、経路探索は
  後続Issueで仕様化する。固定ステップの軸選択はX、Y、Zの優先順で同率時も安定する。
- Verification: AI受入テストを追加し、C++20ビルドと差分検査のみ行う。テスト・成果物・
  サーバは実行していない。
