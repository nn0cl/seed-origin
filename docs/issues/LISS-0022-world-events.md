...
- サーバーの固定ステップは20 FPS（1フレーム50ms）とする。
- 1フレームの受付期間に到着した複数ユーザーのActionを、フレーム開始時に確定する。
- 確定したActionは到着順に一度ずつ処理する。
- フレーム処理中に到着したActionは現在の処理へ割り込まず、次フレームへ繰り越す。
- 到着順を決定できるサーバー側の単調な受付シーケンスを保持する。
