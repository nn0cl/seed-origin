...
サーバーはDisconnectを受理または拒否したあと、同じ16バイトCommandヘッダで **Disconnect ack** を返す（type=`Disconnect`=6）。Acceptedは空payloadと終了したsession ID。Rejectedは非空の理由文字列。クライアントはAccepted ackを見てからauthをAnonymousに戻し、TCPを明示closeする。RejectedではLoggedInのまま、TCPは閉じない。サーバーはAccepted ackをflushしてからconnectionをcloseする。再接続は新しいTCP（`beginReconnect` / `connectTcp`）。Disconnect ackのwireは変えない。
...
`session` は接続同期用でありゲーム内 UI には出さない。`id` は Attack/CastSpell の `targetId` と同じゲーム内ID（クライアントのリモート追跡キー）。`name` は表示専用で必須（空・省略は不正）。認証 PlayerId は載せない。名前の付与・変更は運営経路のみ。Login claimedId を名前にしない。ゲーム内ID と認証 PlayerId は再接続でも変えない。session だけ接続ごとに更新する。
