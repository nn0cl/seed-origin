...
void other_session_keeps_sequence_without_owner_ack() {
    ...
    assert(encode(hazard(2, 2), hazardFrame));
    size_t applied = 0;
    std::string error;
    ...
    assert(owner.expectedSequence() == 3);
    assert(other.expectedSequence() == 3);
    ...
    const client::RemotePlayerPose* remote = other.remotePlayers().find(10);
    assert(remote != 0);
    assert(std::fabs(remote->authority.x - 1.0f) < 0.0001f);
    assert(owner.remotePlayers().find(10) == 0);
}
...
void applies_foreign_public_pose_without_owner_ack() {
    ...
    assert(receiver.receive(frame, applied, error));
    assert(applied == 1);
    const client::RemotePlayerPose* remote = receiver.remotePlayers().find(10);
    assert(remote != 0);
...
void rebases_remote_pose_from_snapshot() {
    ...
    const network::WorldUpdate first = {
        ...
        "ether.fire=0;ether.water=0;ether.earth=0;ether.air=0;ether.hazard=0;"
        "player.count=1;player.0.session=99;player.0.x=1;player.0.y=0;"
        "player.0.z=0"};
    const network::WorldUpdate second = {
        1, network::UpdateKind::Snapshot, 8, 2, 0,
        "ether.fire=0;ether.water=0;ether.earth=0;ether.air=0;ether.hazard=0;"
        "player.count=1;player.0.session=99;player.0.x=10;player.0.y=0;"
        "player.0.z=0"};
    ...
    assert(std::fabs(receiver.remotePlayers().find(99)->authority.x - 1.0f) <
           0.0001f);
    assert(receiver.receive(secondFrame, applied, error));
    const client::RemotePlayerPose* remote = receiver.remotePlayers().find(99);
    assert(remote != 0);
